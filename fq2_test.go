...
func TestFQ2Ordering(t *testing.T) {
	...
	b = b.Add(bls.NewFQ2(bls.FQOne, bls.FQZero))
	if a.Cmp(b) >= 0 {
		t.Error("a >= b after adding to b")
	}
	a = a.Add(bls.NewFQ2(bls.FQOne, bls.FQZero))
	if a.Cmp(b) != 0 {
		t.Error("a != b after adding to a and b")
	}
	b = b.Add(bls.NewFQ2(bls.FQZero, bls.FQOne))
	if a.Cmp(b) >= 0 {
	...
	a = a.Add(bls.NewFQ2(bls.FQZero, bls.FQOne))
	if a.Cmp(b) <= 0 {
...
func TestFQ2Squaring(t *testing.T) {
	...
	a0, _ := new(big.Int).SetString("7080c5fa1d8e04241b76dcc1c3fbe5ef7f295a94e58ae7c90e34aab6fb6a6bd4eef5c946536f6029c2c6309bbf8b598", 16)
	a1, _ := new(big.Int).SetString("10d1615e75250a21fc58a7b7be815407bfb99020604137a0dac5a4c911a4353e6ad3291177c8c7e538f473b3c870a4ab", 16)
	a = bls.NewFQ2(bls.NewFQ(a0), bls.NewFQ(a1)).Square()
...
func TestFQ2Mul(t *testing.T) {
	a0, _ := new(big.Int).SetString("787155314249811040945591987890197698923521563000754191852630526325910580973833767165585004431929413598958418534147", 10)
	a1, _ := new(big.Int).SetString("3701300508704531706796752084315366662585685126320813849768310480839238335689933757701592694115590705658584012352975", 10)
	a := bls.NewFQ2(bls.NewFQ(a0), bls.NewFQ(a1))
	b0, _ := new(big.Int).SetString("428812050294605776242613635112226565923171104978712194146076278968219164256021918826122932548851868482795586147198", 10)
	b1, _ := new(big.Int).SetString("3983398787459011889399209724009438146317670502764434060579092960070359230210191684510008045453936255762435213464276", 10)
	b := bls.NewFQ2(bls.NewFQ(b0), bls.NewFQ(b1))
...
func TestFQ2Addition(t *testing.T) {
	a0, _ := new(big.Int).SetString("149598838772691373030373252224260385709104435267292980765191905060056642409050296541472646296924142271996639838151", 10)
	a1, _ := new(big.Int).SetString("181009733668961021523236619221743585335449215747654796739195280796206588018924306389615593874039575085936328761563", 10)
	a := bls.NewFQ2(bls.NewFQ(a0), bls.NewFQ(a1))
	b0, _ := new(big.Int).SetString("3048378325167389389927636537438764659078261027448144976696012669013445082263630955837031612987742627153430205370098", 10)
	b1, _ := new(big.Int).SetString("2745732935355827219217491212445536316258315122004495924168966599922007114897473294472413581011479900113332636006841", 10)
	b := bls.NewFQ2(bls.NewFQ(b0), bls.NewFQ(b1))
...
func TestFQ2FrobeniusMap(t *testing.T) {
	...
	a1, _ := new(big.Int).SetString("181009733668961021523236619221743585335449215747654796739195280796206588018924306389615593874039575085936328761563", 10)
	a := bls.NewFQ2(bls.NewFQ(a0), bls.NewFQ(a1))
	o00, _ := new(big.Int).SetString("149598838772691373030373252224260385709104435267292980765191905060056642409050296541472646296924142271996639838151", 10)
...
func TestFQ2Sqrt(t *testing.T) {
	a0, _ := new(big.Int).SetString("1199141494453035944462437764039889211157529566989597369793908631238550963749870321413115974274142904836460239708711", 10)
	a1, _ := new(big.Int).SetString("2275726519012059234888434151906712303065680739818771469342471201468687921783347469538129328635084833206431018046147", 10)
	a := bls.NewFQ2(bls.NewFQ(a0), bls.NewFQ(a1))
...
func BenchmarkFQ2Add(b *testing.B) {
	type addData struct {
		f1 *bls.FQ2
		f2 *bls.FQ2
	}
...
func BenchmarkFQ2Sub(b *testing.B) {
	type addData struct {
		f1 *bls.FQ2
		f2 *bls.FQ2
	}
...
func BenchmarkFQ2Square(b *testing.B) {
	type addData struct {
		f1 *bls.FQ2
	}
...
func BenchmarkFQ2Inverse(b *testing.B) {
	type addData struct {
		f1 *bls.FQ2
	}
